...
OpenPrism can run the Agent `Tools` mode through the DeepSeek Harness TypeScript SDK.
...
The LLM API key is taken from the OpenPrism LLM setting, or from
`DEEPSEEK_API_KEY`. The model and endpoint are passed to the Harness SDK as the
provider/model route.

## Safety boundary

Each request is executed in a temporary copy of the OpenPrism project. Harness
may use its configured tools inside that copy, but OpenPrism never exposes the
original project directory to the Harness process. When the run finishes,
OpenPrism converts text-file changes into pending patches. The original project
...
If the Harness SDK cannot start or the model turn fails, OpenPrism falls back
to the existing LangChain agent by default. Set
`OPENPRISM_HARNESS_FALLBACK=false` to fail closed instead.

When the endpoint field is left at OpenPrism's default OpenAI URL, an existing
...
```text
OPENPRISM_HARNESS_PROFILE=sdk
OPENPRISM_HARNESS_DSH_BIN=/absolute/path/to/dsh
OPENPRISM_HARNESS_PROVIDER=deepseek-official
OPENPRISM_HARNESS_MAX_TOKENS=49152
OPENPRISM_HARNESS_TIMEOUT_MS=600000
```
