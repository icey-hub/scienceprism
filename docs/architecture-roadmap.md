...
- [x] 将项目首页升级为项目驾驶舱，展示研究目标、阶段进度、待审批事项、最近运行、风险和下一步。
- [x] 增加首次使用引导和项目初始化流程，覆盖模板、研究问题、模型、约束和第一阶段。
- [x] 建立论文资料库，支持论文导入、去重、标签、收藏、阅读状态、笔记、批注、BibTeX 和来源检查。
- [x] 建立轻量任务中心，接入 Harness、论文导入、编译和实验任务，提供日志、重试和失败详情。
- [x] 建立写作质量面板，接入主张-证据矩阵、引用完整性、术语一致性、编译问题和 AI 检查结果。

### 验收标准

- [x] 用户可以从一个项目驾驶舱理解项目现状和下一步，而不需要逐页寻找。
- [x] 论文从检索到阅读、筛选、证据引用和写作可以在项目内连续完成。
- [x] Harness 和实验等长任务能从任务中心查看日志、重试、取消，并在确有需要时恢复或进入产物页面。
- [x] 关键运行、证据和写作检查结果都能从项目内回查。

### 第七阶段执行记录

- 实际变更：新增 `projectHub` Product Loop Module，提供项目驾驶舱聚合、首次初始化、项目约束写入和下一步/风险投影；新增 `/project/:projectId`、`/library`、`/tasks` 和 `/quality` 工作台视图。
- 论文资料库：新增 `.scienceprism/paper-library.json` 和论文 Interface，支持按 arXiv/DOI/URL/标题去重、标签、收藏、阅读状态、笔记、批注、BibTeX、来源检查，并为每篇论文确保项目 Evidence 引用。
- 任务中心：新增 `.scienceprism/tasks.json`，聚合 Harness Run、研究阶段任务、论文导入、编译和 Experiment Plan；支持日志、失败详情、Harness 重放、取消和重试，Experiment Plan 明确保持计划态，不执行 Shell。
- 写作质量：新增主张-证据、LaTeX 引用完整性、关键词术语变体、编译失败和写作 Harness 检查的统一投影；新建项目从项目列表进入驾驶舱，初始化后再进入研究方向。
- API：新增 `/api/projects/:id/dashboard`、`/initialize`、`/papers`、`/tasks` 和 `/writing-quality` 及其操作接口；编译结果写入任务日志。
- 测试结果：`node --test apps/backend/test/*.test.js` 通过 21 项；`npm run build` 通过。构建仍有既有 SVG 运行时路径提示、`pdfjs-dist` eval 提示和大 bundle warning。
- 遗留风险：来源检查目前是本地元数据完整性检查，不替代外部数据库核验；任务记录和资料库仍使用单进程文件锁；写作质量中的 AI 检查需要已有 Writing Harness 结果，阶段八实验执行和产物页面仍未开放。
