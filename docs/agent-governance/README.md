...
- **Round 4 进度**：031 ✅ **加**：收口 C-09（网络 allowlist 由 fail-open 改为 fail-closed）；032 ✅ **减**：收口 C-11（不确定性收敛为**一条强制通道**，点名即接受、未点名即拒）；033 ✅ **验证**：门禁 exit 0、运行时直证修复生效、零依赖变更；下一步 034（收口 C-10）。
- **剩余漂移 2 条**：C-04（actor 自报）、C-10（legacy 适配器不接收 limits）。
- **溯源改善**：**AI 主观添加的约束从 2 条降到 1 条**（只剩 C-16）；core 13 → 14；测试 90 → 92。
- **Round 4 其余目标**：R-15 `aidoc/` 落点策略、角色在前端展示、失败重试回灌。
- 会话内 goal：`goal-16c676ec`（armed，200 轮预算，已用 33 拍）。
