...
- **Round 4 进度**：031 ✅ **加**：收口 C-09——发现并修掉网络 allowlist 的 **fail-open**（空白名单曾放行任意主机，而提示词告诉模型 "none"），漂移 **4 → 3**；下一步 032（减：C-11 不确定性字段收敛为一条强制通道）。
- **剩余漂移 3 条**：C-04（actor 自报）、C-10（legacy 适配器不接收 limits）、C-11（不确定性字段全为可选）。
- **Round 4 其余目标**：R-15 `aidoc/` 落点策略、角色在前端展示、失败重试回灌。
- 会话内 goal：`goal-16c676ec`（armed，200 轮预算，已用 31 拍）。
