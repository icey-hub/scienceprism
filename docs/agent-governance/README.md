...
- **Round 2 进度**：011 ✅ 约束注册表骨架 + 审计文档；012 ✅ 约束可选开关；013 ✅ 收口漂移（9 → 5）+ 16/16 全有测试；014 ✅ 验证；015 ✅ **收口 4 处高危绕过**（plot 真高危已 fail-closed，transfer/vision 变为可拒绝，llm 据实降级）；下一步 016（验证 015 + 角色注册表起头）。测试 51 → **67 项全绿**。
