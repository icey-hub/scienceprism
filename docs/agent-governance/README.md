...
- **Round 4 进度**：031 ✅ **加**：收口 C-09（网络 allowlist 由 fail-open 改为 fail-closed）；032 ✅ **减**：收口 C-11（不确定性收敛为**一条强制通道**，点名即接受、未点名即拒）；033 ✅ **验证**：门禁 exit 0、运行时直证修复生效、零依赖变更；034 ✅ **加**：收口 C-10（token 预算现在传给**每个**适配器）；下一步 035（收口 C-04）。
- **剩余漂移 1 条**：C-04（actor 自报，无法区分 AI 与人类）。
- **溯源改善**：AI 主观添加的约束 **2 → 1**（只剩 C-16）；测试 90 → **94 全绿**。
- **Round 4 其余目标**：R-15 `aidoc/` 落点策略、角色在前端展示、失败重试回灌。
- 会话内 goal：`goal-16c676ec`（armed，200 轮预算，已用 34 拍）。
