...
**阶段：本地质量整合已完成迭代 065；实验数据、项目文件边界和项目概览提示已复核。此前 Round 6 的历史记录见下。**

| # | 迭代 | 状态 | 验证证据 |
| --- | --- | --- | --- |
| 065 | 本地质量整合：补齐 CoT 实验失败单元、隔离自洽性实验检查点、修项目路径边界与概览提示 | ✅ 完成 | `npm run quality`：126 项测试通过、类型检查通过、构建通过、6 张图 0 布局缺陷；Codex 内置浏览器走通创建项目、初始化、概览、编辑器；论文 PDF 已重建 |

- **实验产物**：CoT 实验 600/600 单元完整，`format#173` 的 HTTP 502 已单独补测并保留修复记录；实际回答错误，因此准确率不变，解析失败数由 2 修正为 1。派生分析、图、写作 Brief、手稿和 PDF 已同步。
- **未完成的自洽性实验**：现有 3 条部分记录保存在仓库内 `.cache/experiment-self-consistency-checkpoint.json`，不作为正式结论；同参数运行脚本可续传，完成后才会发布到 `aidoc/`。
- **本地产品检查**：阻止项目 ID、模板 ID 与符号链接把文件操作带出项目目录，也阻止 `path=.` 删除整个项目；概览在没有 Harness 运行时不再声称 AI 输出已完成。

- **Round 5 收尾**：对比文档 `rounds/round-05-comparison.md`；tag `round-05-complete`；分支 `feat/agent-governance-r5` 推 `scienceprism`。
- **Round 5 关键指标**：后端测试 **99 → 109**；约束表由**手写改为整表生成**；阶段词汇定义处 **2 → 1**；敏感目录表 **2 → 1**；"已结束的 Run"判定 **4 处调用点 → 1 个常量**；新增**角色可见性**与**失败重试回灌**；漂移保持 **0**；**零依赖变更**。
- **✅ 约束治理闭环**：人面向清单、机器审计、强制点、测试引用、溯源、漂移——**每一面都是代码投影**，手改文档会被门禁拦下并提示改注册表。
- **Round 6 已完成（051–058）**：① 整理项目并修复我把 `pic/`（149MB）误提交的事故；② `pic/` 入库为 0、`.git` 153M → 39M；③ 安装绘图 skill（9 个产品 skill）；④ 修 frontmatter 块标量解析（描述不再变成 `">-"`）；⑤ **AI 方向论文**：GSM8K 思维链消融，**600 次真实生成**；⑥ 修产品缺陷"写作阶段看不到证据账本"；⑦ 论文配图全英文重做；⑧ 图的可复现/数据现算/布局三条门禁。
- **⚠️ 节奏如实说明**：Round 6 的"加/减/验证"三拍循环**被打断两次**（用户中途改方向）。051–058 按用户当次指令推进，**没有为凑节奏伪造拍型**。
- **Round 6 遗留**：论文结构门禁（实证论文须有 Results 章节、须有引用结果数据的图、附录不得有无关图）；实验扩到多模型 / 多次生成 / 自洽性对照。
- 会话内 goal：`goal-16c676ec`（armed，200 轮预算，已用 58 拍）。
...
| Round 5（041–050） | `feat/agent-governance-r5` | `round-05-complete` | `rounds/round-05-comparison.md` | 测试 99 → 109；**约束表由手写改为整表生成**（约束治理闭环）；角色可见性（前端面板 + `GET /api/agent/roles`）；失败重试回灌（有界、串行、可审计）；阶段词汇 2 → 1；敏感目录表 2 → 1；"Run 已结束"4 处调用点 → 1 常量（并判定 taskCenter 不该合并）；漂移保持 0；零依赖变更 |
