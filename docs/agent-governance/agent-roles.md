...
| `research-stage-assistant` | One structured producer per Research Stage; JSON output only. | suggest-only | direction, search, selection, replication, ideation, method, experiment, writing | `project.read` | `literature-search`, `paper-screening`, `paper-card`, `dataset-audit`, `statistics-audit`, `research-writing` |
