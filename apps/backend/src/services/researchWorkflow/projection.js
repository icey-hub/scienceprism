...
export function toFrontendWorkflow(workflow) {
  ...
  return {
    ...
    experiment: { dataset: experiment.dataset || experiment.datasetId || experiment.datasetIds?.join(', ') || '', command: experiment.command || experiment.commands?.join('\n') || '', status: experiment.status || '待规划', metrics: experiment.metrics || [] },
